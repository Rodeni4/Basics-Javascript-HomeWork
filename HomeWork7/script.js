...
let elementsA = document.querySelectorAll('.dropdown-item');
...
let elementBtn = document.querySelector('.btn');// if
elementBtn.classList.toggle('btn-secondary');

// 3. Необходимо удалить класс "dropdown-menu" у элемента, у которого присутствует класс "menu".
let elementMenu = document.querySelector('.menu');
elementMenu.classList.remove('dropdown-menu');

// 4. Используя метод insertAdjacentHTML добавьте после div'a с классом "dropdown" следующую разметку:
// `<a href="#">link</a>`
let elementDiv = document.getElementsByClassName('dropdown')[1];
elementDiv.insertAdjacentHTML('afterEnd', `<a href="#">link</a>`);

// 5. У элемента с id "dropdownMenuButton" замените id на "superDropdown".
let elementId = document.getElementById('dropdownMenuButton');
elementId.id = 'superDropdown';
...
let elementData = document.querySelector('[aria-labelledby="dropdownMenuButton"]');
elementData.dataset.dd = '3';

// 7. Удалите атрибут type у элемента с классом "dropdown-toggle".
let elementType = document.querySelector('.dropdown-toggle');
elementType.removeAttribute('type');